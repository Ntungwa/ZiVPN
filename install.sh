...
# =========================
# INSTALL BADVPN-UDPGW (ARCH-SPECIFIC)
# =========================
if [[ "$ARCH" == "x86_64" || "$ARCH" == "amd64" ]]; then
    print_task "Downloading badvpn-udpgw (AMD64)"
    wget -q "https://raw.githubusercontent.com/daybreakersx/premscript/master/badvpn-udpgw64" -O /usr/local/bin/badvpn-udpgw || print_fail "Downloading badvpn-udpgw"
    chmod +x /usr/local/bin/badvpn-udpgw
    print_done "badvpn-udpgw installed to /usr/local/bin"
elif [[ "$ARCH" == "aarch64" || "$ARCH" == "arm64" || "$ARCH" == "armv7l" ]]; then
    print_task "Installing badvpn from apt (ARM)"
    apt-get install -y badvpn &>>"$LOG_FILE" || print_fail "Installing badvpn via apt"
    ln -sf /usr/bin/badvpn-udpgw /usr/local/bin/badvpn-udpgw
    print_done "badvpn-udpgw installed via apt (symlinked to /usr/local/bin)"
else
    print_fail "Unsupported architecture for badvpn-udpgw"
fi
...
Type=simple
User=root
ExecStart=/usr/local/bin/badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 1000 --max-connections-per-client 10
Restart=always
RestartSec=3
LimitNOFILE=65535
