...
run_silent "Installing base deps" "apt-get install -y curl wget openssl ca-certificates net-tools iptables iptables-persistent netfilter-persistent zip unzip cron badvpn"
ln -sf /usr/bin/badvpn-udpgw /usr/local/bin/badvpn-udpgw 2>/dev/null || true
...
Type=simple
User=root
ExecStart=/usr/bin/badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 1000 --max-connections-per-client 10
Restart=always
RestartSec=3
LimitNOFILE=65535
